...
{  
	...
	var numberOfRoundsAfterCycle = -1;
	...
	this.move = function()
	{
		...
		if(this.estado < 0)
		{
			...
			this.recommendationHistory.push(this.latestChoice);
	...
	this.getCurrentStateMachine = function()
	{
		...
		{// best response
			return this.br.getStateMachine();
	...
	this.isSatisfied = function()
	{
		...
		var vale = numberOfRoundsAfterCycle % 2
		var acts = this.gameHistory[this.gameHistory.length - 1];
		var gotten = this.M[this.me][acts[0]][acts[1]];

		if((this.experto % 2) == 0)
		{ // leader part of algorithm
			var aspArray = this.re[(this.experto - 2) / 2].getMyAspirationForARound();
		}
		else
		{ // follower part of algorithm
			var aspArray = this.re[Math.floor((this.experto - 2)/ 2)].getMyAspirationForARound();
		}
